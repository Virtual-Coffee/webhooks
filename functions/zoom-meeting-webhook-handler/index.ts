...
import { updateMeetingStatus, updateMeetingAttendence } from './slack.js';
import { findRoomInstance } from './airtable.js';
import type { Room } from '../../types/room.js';

// @ts-expect-error - rooms.json is generated by the build script before bundling
...
export default async (req: Request) => {
  try {
    ...
    if (zmSignature) {
      const zmTimestamp = req.headers.get('x-zm-request-timestamp');
    ...
    console.log(error);
    return new Response(
      JSON.stringify({ msg: error instanceof Error ? error.message : String(error) }),
      { status: 500 },
    );
  }
};
